...
/* ------------------------------------------------------------
   liff.getProfile() リトライ付きラッパー
   ログインリダイレクト直後など、トークンの準備が間に合わず
   一時的に失敗することがあるため、少し待って数回リトライする。
   ------------------------------------------------------------ */
async function getProfileWithRetry(retries = 2, delayMs = 500) {
  let lastErr;
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      lastErr = e;
      console.warn(`liff.getProfile failed (attempt ${i + 1}/${retries + 1}):`, e);
...
/* ============================================================
   hideFormElements() / showStateCard() の巻き戻し。
   「共有リンクかもしれない」と判定して先にローディング表示を
   出したものの、実際には共有リンクではなかった（通常の回答
   フォームだった）場合に、隠したフォーム要素と viewMode の
   ローディング表示を元に戻すために使う。
   ============================================================ */
...
async function handleSharedView(id) {
  ...
  try {
    ...
    viewerHash = await sha256Hex(profile.userId);
  } catch (e) {
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const HEADER_IMAGE_URL = "https://liffdevelop31257014-gif.github.io/-jikokaiji_qa2/image_message.jpg"; 
...
(async () => {
  ...
  /* ----- 共有リンクの可能性がある場合は先に簡易ローディング表示 -----
     liff.init() の完了を待たずに location.search / location.hash を
     読んでしまうと、LINEログインのリダイレクト復帰直後は
     ?liff.state=... のような一時的なURLになっていて ?id=... を
     正しく読み取れず、誤って質問フォームが表示されてしまうことが
     あった。そのため、判定に使うURLの読み取りは liff.init() の
     完了後に行う。ただし完全に何も表示しないと真っ白な一瞬が
     生じるため、それらしき兆候（id= や #以降のフラグメント、
     liff.state）が見えている場合だけ先にローディング表示を出す。
  ----- */
  const mightBeSharedView =
    location.search.includes("id=") ||
    location.search.includes("liff.state") ||
    location.hash.length > 1;
  if (mightBeSharedView) {
    showStateCard("読み込み中…", "回答内容を確認しています。少々お待ちください。", true);
  }
  ...
  /* ----- 共有リンク判定（?id=... が付いている場合） -----
     liff.init() 完了後に読むことで、ログインリダイレクト経由で
     戻ってきた場合でも元の ?id=...#key が正しく復元された状態で
     判定できる。
  ----- */
  ...
  // 共有リンクではないと判明したので、念のため出していたローディング
  // 表示があれば元のフォーム表示に戻す。
  if (mightBeSharedView) {
    showFormElements();
  }
  ...
  document.getElementById("shareBtn").addEventListener("click", async () => {
    ...
    try {
      const profile   = await getProfileWithRetry();
      const ownerHash = await sha256Hex(profile.userId);
